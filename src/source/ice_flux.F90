!  SVN:$Id: ice_flux.F90 915 2015-02-08 02:50:33Z tcraig $
!=======================================================================
...
! 2006: Converted to free source form (F90) by Elizabeth Hunke
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,max_blocks), public :: &
         ...
         Tsnice , & ! snow ice interface temperature   (C)
...
! author Elizabeth C. Hunke, LANL
      ...
      fiso_rain (:,:,:,:) = c0           ! isotopic rain rate (kg/m2/s)
...
! author: Elizabeth C. Hunke and William H. Lipscomb, LANL
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block), intent(in):: &
          ...
          Cd_atm_n, & ! ratio of total drag over neutral drag (atm)
          ...
          Tsnicen  , & ! snow ice interface temperature  (C)
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block), &
          ...
          Cd_atm, & ! ratio of total drag over neutral drag (atm)
...
!ocl novrec      !Fujitsu
      do ij = 1, icells
         ...
         Cd_atm (i,j) = Cd_atm (i,j) + &
                                   Cd_atm_n (i,j)*aicen(i,j)
         ...
         Tsnice    (i,j)  = Tsnice   (i,j) + Tsnicen  (i,j)*aicen(i,j)
